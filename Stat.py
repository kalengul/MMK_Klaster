...
def SaveStatisticsExcel(NameFile,time,koliter,P):
    ...
    i=0
    while i<len(ArrTime): 
        sheet.Cells(NomR,34+i+7*lenProcIS+len(NomElGraphTree)+12).value =str(ArrTime[i])
        i=i+1
...
def SaveTime(timeDuration):
   ArrTime.append(timeDuration) 
   return 0 
...
def SaveParametr(NameFile,N,Ro,Q,alf1,alf2,koef1,koef2,typeProbability,NameFileXL,AddFeromonAntZero,SbrosGraphAllAntZero,goNewIterationAntZero,goGraphTree,SortPheromon,KolIteration,KolStatIteration,MaxkolIterationAntZero):
    Excel = win32com.client.Dispatch("Excel.Application")
    wb = Excel.Workbooks.Open(NameFile)
    sheet = wb.ActiveSheet 
    NomR=sheet.Cells(1,1).value
